...
function cacheKey(provider: string, model: string, apiKey?: string): string {
  const baseKey = `${provider.trim().toLowerCase()}:${model.trim()}`;
  if (!apiKey) return baseKey;
  // Use a secure hashing algorithm (PBKDF2) to satisfy CodeQL's password hashing requirements
  const hashedKey = pbkdf2Sync(apiKey, 'salt', 100_000, 64, 'sha512').toString('hex');
  return `${baseKey}:${hashedKey}`;
}
...
export function resolveWorkerModel(
  ...
  const key = cacheKey(override.provider, override.model, override.apiKey);
