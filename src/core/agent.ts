...
Use your tools to inspect implementation details, verify assumptions, and produce precise security findings.`,
  ...
  async sendMessage(
    ...
    // LangGraph manages conversational memory internally,
    // but we maintain the artifact record for the run directory log.
    ...
    emitEvent({ kind: 'status', message: 'Planning analysis mission via LangGraph' });
    ...
    try {
      ...
      // NOTE: Ensure your compiled graph is assigned to `this.workflow` during initialization.
      const config = { configurable: { thread_id: this.artifacts.getRunDirectory() } };

      const eventStream = await this.workflow.streamEvents({
        messages: [{ content: userMessage, role: 'user' }]
      }, { version: 'v2', ...config });

      // Bridge LangGraph events to the existing React Ink UI callbacks
        ...
        switch (eventType) {
          case 'on_chat_model_stream': {
            ...
            if (chunk && typeof chunk === 'string') {
              ...
              onChunk(chunk); // Feeds the UI streaming buffer
            }
      ...
      await this.artifacts.writeReportMarkdown(finalResponseText);

      // Retrieve the final structured state from LangGraph
      const finalState = await this.workflow.getState(config);
      const rawFindings = finalState.values?.findings || [];

      // Because LangGraph tools guarantee structured output, we bypass the old `generateText` repair loop
      const dedupedReport: SecurityReport = {
        findings: deduplicateFindings(rawFindings),
      };
      ...
      // Preserve CI/CD exit code logic
      ...
      this.runtimeWarnings.push(`LangGraph execution failed: ${(error as Error).message}`);
    }

    if (missionActionId) {
        await this.completeMissionCycle(missionActionId, validatedReport, finalResponseText.length);
    }
  ...
  private async initialize(): Promise<void> {
    ...
    this.workflow = compileWorkflow(toolsArray);
